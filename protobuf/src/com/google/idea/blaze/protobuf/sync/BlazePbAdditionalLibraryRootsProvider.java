...
import com.google.idea.blaze.protobuf.resolve.BazelProtoPackage;
...
public final class BlazePbAdditionalLibraryRootsProvider extends BlazeExternalLibraryProvider {
  private static final BoolExperiment usePbAdditionalLibraryRootsProvider =
      new BoolExperiment("use.proto.additional.library.roots.provider4", true);

  @Override
  protected String getLibraryName() {
    return "Protobuf Libraries";
  }

  @Override
  protected ImmutableList<File> getLibraryFiles(Project project, BlazeProjectData projectData) {
    if (!usePbAdditionalLibraryRootsProvider.getValue()) {
  ...
  static ImmutableList<File> getLibraryFiles(
    ...
    if (!projectData.getWorkspaceLanguageSettings().isLanguageActive(LanguageClass.PROTO)) {
      return ImmutableList.of();
    }
    ...
    return BazelProtoPackage.getUncachedTargetToFileMap(project, projectData).values().stream()
        .filter(isExternal)
        .filter(f -> f.getName().endsWith(".proto"))
        .distinct()
